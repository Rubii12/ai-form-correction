...
def process_frame(frame, pose_landmarks, mp_pose):
    ...
    if status == "crunched":
        if vel < -80:
            suggestions.append("Slow down — use abs, not momentum.")
            rep_quality = "okay"
        elif not suggestions:
            suggestions.append("Hold the crunch 1 second — squeeze abs.")
    elif status == "extended":
        if aint > 140:
            suggestions.append("Don't fully relax at bottom — keep tension.")
            rep_quality = "okay"
        elif not suggestions:
            suggestions.append("Controlled lower — don't flop down.")
    elif status == "mid":
        if not suggestions:
            suggestions.append("Exhale as you crunch up.")

    if aint < 30:
        suggestions.append("Avoid pulling on your neck — hands lightly behind head.")
