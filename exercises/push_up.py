...
def process_frame(frame, pose_landmarks, mp_pose):
    ...
    if status == "down":
        if aint > 75:
            suggestions.append("Lower chest closer to the floor.")
            rep_quality = "needs work"
        if vel < -120:
            suggestions.append("Control the descent — don't drop.")
            rep_quality = "okay"
        elif not suggestions:
            suggestions.append("Elbows at ~45° from body — not flared.")
    elif status == "up":
        if aint > 170:
            suggestions.append("Soft elbows at the top — avoid locking out.")
        elif not suggestions:
            suggestions.append("Full extension — push the floor away.")
    elif status == "mid":
        if not suggestions:
            suggestions.append("Keep core tight — no sagging hips.")

    if aint < 40:
        suggestions.append("Keep neck neutral — eyes to the floor.")
        rep_quality = "needs work"
    ...
    if _STATE["tick"] == 0:
        print(f"[push_up] sm={_STATE['smoothed']:.1f} a={aint} vel={vel:.1f} in_down={_STATE['in_down']} reps={_STATE['rep_count']}")
